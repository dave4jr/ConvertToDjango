...
class CreateURLCONF(InitVariables):
	def run(self):
		...
		themes =  self.THEME_FILES

		base_templates = []
		for template in templates:
			base_template = template[(len(self.TEMPLATE_DIR)+1):len(template)]
			base_templates.append(base_template)
		
		base_themes = []
		for theme in themes:
			base_theme = theme[(len(self.THEME_DIR)+1):len(theme)]
			baseUS = base_theme.replace("/","_")
			baseDash = baseUS.replace("_", "-")
			base_themes.append(baseDash)

		pp(base_themes)

		# for template_file in base_templates:
		# 	if template_file not in base_themes:
		# 		print template_file
		# 		break


		# for template_file in self.TEMPLATE_FILES:
		# 	folder = self.THEME_FOLDERS[self.COUNTER]

		# for folder in self.THEME_FOLDERS:
		# conf = "url(r'^%s/$', '%s.views.navigation', {\"url\":\"%s\"}, name=\"%s_%s\")," % (url_1_3, self.APP_NAME, url_1_2, self.APP_NAME, url_1_3)
		# print conf
