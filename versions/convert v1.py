...
class InitVariables():
	def __init__(self):
		...
		self.APP_NAME				= "jango"																		# This is the name of the app name and folder name
		self.THEME_DIR				= "/sys/centerstack/materials/templates/jango/templates"										# Enter the directory containing HTML files from the original theme
...
class ConvertToDjango(InitVariables):
	...
	def compile_meta(self):
		try:
			...
			print "[!]  Note: meta tag for author description does not exist in this HTML Template. "
		...
		for attribute in self.LINK_ATTRIBUTES:
			...
			for attr in attrs:
				...
				if os.path.isfile(os.path.join(self.STATIC_DIR, tag)):
					...
					# if self.THEME_FOLDERS[self.COUNTER] == "root":
					# 	urlUS = "%s_%s" % (self.APP_NAME, f)
					# else:
						# urlUS = "%s_%s_%s" % (self.APP_NAME, self.THEME_FOLDERS[self.COUNTER], f)
					# url = urlUS.replace("/","_").replace("_", "-")
					# url = "%s-%s" % (self.APP_NAME, self.THEME_TRANSFERNAMES[self.COUNTER])
					# attr[attribute] = "{%% url '%s' %%}" % url
				else:
					pass
				print tag
				# print "%s ----- %s ----- %s ----- %s" % (tag, url, self.THEME_FOLDERS[self.COUNTER], self.COUNTER)
