...
class InitVariables():
	...
	def compile_links_loop(self):
		for attribute in self.LINK_ATTRIBUTES:
			tags = self.soup.find_all(attrs = {attribute: True})

			for tag in tags:
				tag_value = tag[attribute]
				if tag_value[0:3] == "../":
					tag_value = tag_value.replace("../", "")
				try:
					f, ext = os.path.splitext(tag_value)
				except:
					pass

				if os.path.isfile(os.path.join(self.STATIC_DIR, tag_value)):
					tag[attribute] = "{%% static '%s/%s' %%}" % (self.APPNAME, tag_value)
				elif ext == ".html":
					if self.URL_PREFIX == 0 or self.URL_PREFIX == "":
						tag[attribute] = "{%% url '%s' %%}" % f
					else:
						tag[attribute] = "{%% url '%s%s' %%}" % (self.URL_PREFIX, f)
				else:
					pass
